...
static int next_line = 1; // 定义全局变量(CH_bool类型) next_line 为 1(真)， 用于保存是否需要自动换行
static FILE* logfile; //创建文件流指针用于IO操作
static char nxt[3][2] = {"","","\n"};// 创建字符表 用于替换复杂的比较操作
...
void CH_log_next_line_have_to_is(int CHbool){  // 定义函数 CH_next_line_haveto_is 原型是用于强制设置设置next_line的值
...
void CH_log_partion(){ // 定义函数 CH_log_partion 是用于分割日志文件
    fprintf(logfile,"\n\n\n"); // 写入分割
}

// main----------------------------------------------------------------------------------------------------
void CH_log_build_time(){// 获取时间并保存到全局变量的文件内
    time_t now = time(NULL); // 获取当前时间
    fprintf(logfile, "%s", ctime(&now)); // 写入文件
}

void CH_log_ubuild_time(FILE* fileptr){//获取时间并保存到用户提供的文件内
    time_t now = time(NULL); // 获取当前时间
    fprintf(fileptr, "%s", ctime(&now)); // 写入文件
}

void CH_log_afbuild(char* head,char *msg){//取消了原来在调用时自动记录时间， 
    fprintf(logfile, "[%s]: %s%s", head, msg, nxt[(msg[strlen(msg)-1] != '\n')+next_line]);
    // 写入自定义信息并自动换行
}
void CH_log_afuse(char* head,char *msg, FILE* log_file){//取消了原来在调用时自动记录时间， 
